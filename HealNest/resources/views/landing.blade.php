...
<body>
    ...
    <section class="min-h-screen flex items-center px-6 py-20 max-w-6xl mx-auto">
        <div class="grid md:grid-cols-2 gap-12 items-center w-full">
            ...
            <div class="grid grid-cols-2 gap-4">
                @foreach([['😊','Daily Mood Tracking','Log how you feel every day'],['📋','PHQ-9 & GAD-7','Evidence-based assessments'],['📈','Progress Charts','Visualize your journey'],['👩‍⚕️','Counselor Support','Professional guidance']] as $card)
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-tan/20 hover:shadow-md transition-shadow">
                    <div class="text-3xl mb-2">{{ $card[0] }}</div>
                    <h3 class="font-heading font-semibold text-forest text-sm">{{ $card[1] }}</h3>
                    <p class="text-gray-500 text-xs mt-1">{{ $card[2] }}</p>
                </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="grid md:grid-cols-3 gap-8">
                ...
                <div class="p-6 rounded-2xl border border-tan/20 hover:border-midgreen transition-colors">
                    ...
                    <h3 class="font-heading font-semibold text-forest text-lg mb-2">{{ $f[1] }}</h3>
    ...
    <footer class="bg-forest/90 text-green-200 text-center py-6 text-sm">
        © {{ date('Y') }} HealNest. Built with care for mental wellness.
    </footer>
</body>
</html>
